...
} from "@/src/app/actions/interview";
  ...
  return (
    <div className="mx-auto grid max-w-4xl gap-4 sm:gap-6 lg:grid-cols-[1fr_320px] font-sans">
      <div className="flex flex-col rounded-2xl sm:rounded-3xl border border-zinc-200 bg-white p-4 sm:p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 transition-all duration-500 hover:shadow-md hover:border-zinc-300/40 dark:hover:border-zinc-700/60">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">
                Let&apos;s map out your venture together.
              </p>
            </div>
          </div>
        </div>
        ...
        <div
          ref={chatListRef}
          className="flex-1 overflow-y-auto py-4 sm:py-6 space-y-4 max-h-[380px] sm:max-h-[450px] min-h-[280px] sm:min-h-[350px] pr-1"
        >
          {messages.map((msg) => (
            <div
              key={msg.id}
          ...
          {submitting && (
            <div className="flex justify-start animate-pulse">
              <div className="flex items-center gap-2 max-w-[80%] rounded-2xl bg-zinc-50 dark:bg-zinc-800 px-5 py-3.5 text-xs text-zinc-400 font-medium">
                <span className="size-4 animate-ping text-green-700" />
                Thinking...
              </div>
            </div>
          )}
        ...
        {!isCompleted ? (
          <form
            onSubmit={handleSend}
            className="border-t border-zinc-100 pt-4 dark:border-zinc-800 flex gap-3"
          >
            <input
              type="text"
              value={currentAnswer}
          ...
          <div className="border-t border-zinc-100 pt-6 dark:border-zinc-800 text-center">
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Your custom dashboard and business briefs are ready.
            </p>
            <button
      ...
      <aside className="space-y-4">
        <div className="rounded-2xl sm:rounded-3xl border border-zinc-200 bg-white p-4 sm:p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 transition-all duration-500 hover:shadow-md">
          ...
          <p className="mt-1 text-xs text-zinc-500 leading-relaxed">
            We need a bit of details to start generating recommendations.
          </p>
          <div className="mt-5">
