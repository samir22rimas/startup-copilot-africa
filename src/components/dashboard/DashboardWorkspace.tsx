  ...
  return (
    <div className="space-y-7 pb-12">
      ...
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          <p className="text-sm font-semibold text-green-700">Founder workspace &mdash; {startup.name}</p>
        ...
        <button
          ...
          className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-green-700 px-4 text-sm font-semibold text-white shadow-lg shadow-green-900/15 transition hover:bg-green-800"
        >
          <Sparkles className="size-4" /> Ask your copilot
        </button>
      </div>

      {/* Progress & Focus Metrics */}
      <section className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
        <div className="relative overflow-hidden rounded-3xl bg-[#0b3327] p-7 text-white shadow-xl sm:p-8">
          <div className="absolute -right-14 -top-16 size-56 rounded-full bg-green-400/15 blur-2xl" />
          <div className="relative flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between">
            <div className="max-w-sm">
              <div className="flex size-11 items-center justify-center rounded-2xl bg-green-400/15 text-green-300">
            ...
            <div className="flex size-36 shrink-0 items-center justify-center rounded-full border-[11px] border-green-400/20 bg-white/5 text-center">
              <div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-7 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-zinc-900 dark:text-white">Market adaptation focus</p>
              <p className="mt-1 text-sm text-zinc-500">Milestones aligned with local conditions.</p>
            </div>
            <Target className="size-5 text-green-700" />
          </div>
          <div className="mt-6 space-y-4">
            <div>
              <div className="mb-2 flex justify-between text-xs font-medium text-zinc-600 dark:text-zinc-300">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:p-7">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-bold text-green-700 dark:bg-green-950/40 dark:text-green-400">
          ...
          <div className="mt-6 divide-y divide-zinc-100 dark:divide-zinc-800">
            {tasks.map((task) => (
              <label key={task.id} className="flex cursor-pointer items-center gap-4 py-4 first:pt-0">
                <input
                  type="checkbox"
                  checked={task.done}
                ...
                />
                <span className="flex size-5 items-center justify-center rounded-md border-2 border-zinc-300 text-white transition peer-checked:border-green-600 peer-checked:bg-green-600">
                  <Check className="size-3.5" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className={`block text-sm font-medium ${task.done ? "text-zinc-400 line-through" : "text-zinc-800 dark:text-zinc-100"}`}>
                    {task.title}
                  </span>
                  <span className="mt-1 inline-block rounded bg-zinc-100 px-2 py-0.5 text-[11px] font-medium text-zinc-500 dark:bg-zinc-800">
                    {task.tag}
                  </span>
                </span>
          ...
          {!showAddTask ? (
            <button
              ...
              className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-green-700 transition hover:text-green-800"
            >
              <Plus className="size-4" /> Add custom milestone
            </button>
          ) : (
            <form onSubmit={handleAddTask} className="mt-4 flex flex-col gap-3 sm:flex-row">
              <input
                type="text"
                required
                value={newTaskTitle}
                ...
                className="h-10 min-w-0 flex-1 rounded-xl border border-zinc-200 bg-white px-3 text-sm outline-none transition focus:border-green-600 dark:border-zinc-700 dark:bg-zinc-950"
              />
              <select
                value={newTaskTag}
              ...
              <div className="flex gap-2">
                <button type="submit" className="h-10 rounded-xl bg-green-700 px-4 text-sm font-semibold text-white hover:bg-green-800">
                  Save
                </button>
                <button type="button" onClick={() => setShowAddTask(false)} className="h-10 rounded-xl border border-zinc-200 px-4 text-sm font-medium text-zinc-600 dark:border-zinc-700">
                  Cancel
                </button>
              </div>
            </form>
        ...
        <aside className="space-y-5">
          <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <p className="mt-4 text-sm font-medium leading-6 text-zinc-800 dark:text-zinc-100">
          ...
          <div className="rounded-3xl bg-[#eaf5ee] p-6">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-green-700">Business Health Score</p>
              <h2 className="mt-1 text-3xl font-semibold tracking-tight text-zinc-950 dark:text-white">{healthScore}/100</h2>
              <p className="mt-2 text-sm text-zinc-500">{healthLabel}</p>
            </div>
            <div className="rounded-2xl bg-green-50 px-3 py-2 text-sm font-semibold text-green-700 dark:bg-green-950/40 dark:text-green-300">
              {progress}% roadmap complete
            </div>
          </div>
          <div className="mt-5 h-2 overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-800">
            <div className="h-full rounded-full bg-green-600" style={{ width: `${healthScore}%` }} />
          </div>

          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl bg-zinc-50 p-4 dark:bg-zinc-800/70">
              <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Current business</p>
              <p className="mt-2 font-semibold text-zinc-900 dark:text-white">{startup.name}</p>
            ...
            <div className="rounded-2xl bg-zinc-50 p-4 dark:bg-zinc-800/70">
              <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Active focus</p>
              <p className="mt-2 font-semibold text-zinc-900 dark:text-white">{project.description || "Project details are being refined"}</p>
              <p className="mt-2 text-sm text-zinc-500">
        ...
        <div className="rounded-3xl border border-zinc-200 bg-gradient-to-br from-green-700 to-emerald-800 p-6 text-white shadow-sm">
          <div className="flex items-center gap-2">
          ...
          <div className="mt-5 space-y-3">
            {recommendations.map((recommendation) => (
              <div key={recommendation.title} className="rounded-2xl bg-white/10 p-3">
                <p className="font-semibold">{recommendation.title}</p>
                <p className="mt-1 text-sm text-green-50/85">{recommendation.detail}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="mt-5 space-y-3">
            {documents.map((document) => (
              <div key={document.id} className="flex items-start justify-between gap-3 rounded-2xl bg-zinc-50 p-3 dark:bg-zinc-800/70">
                <div>
                  <p className="font-semibold text-zinc-900 dark:text-white">{document.title}</p>
                  <p className="mt-1 text-sm text-zinc-500">{document.summary}</p>
                </div>
                <span className="rounded-full bg-green-50 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-green-700 dark:bg-green-950/40 dark:text-green-300">
                  {document.status}
                </span>
              </div>
            ))}
          ...
          <div className="mt-5 space-y-3">
            {quickActions.map((action) => (
              <Link
                key={action.label}
                href={action.href}
                className="flex items-center justify-between rounded-2xl border border-zinc-200 bg-zinc-50 p-3 transition hover:border-green-600 hover:bg-green-50 dark:border-zinc-700 dark:bg-zinc-800/70 dark:hover:bg-green-950/20"
              >
                <div>
                  <p className="font-semibold text-zinc-900 dark:text-white">{action.label}</p>
      ...
      <section
        id="copilot"
        className="rounded-3xl border border-green-100 bg-white p-6 shadow-sm dark:border-green-950 dark:bg-zinc-900 sm:p-7"
      >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-500">Your thinking partner for the next decision.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Message history */}
        <div className="mt-5 space-y-4 max-h-[300px] overflow-y-auto border-t border-zinc-100 pt-4 dark:border-zinc-800">
          {copilotMessages.filter(m => m.role !== "system").length === 0 && !copilotLoading && (
            <div className="rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-500 dark:border-zinc-700 dark:bg-zinc-800/60">
          ...
          {copilotMessages.filter(m => m.role !== "system").map((msg) => (
            <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] rounded-xl p-3 text-sm ${msg.role === "user" ? "bg-green-700 text-white" : "bg-zinc-50 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200"}`}>
                {msg.content}
              </div>
            </div>
          ))}
          {copilotLoading && (
            <div className="flex justify-start">
              <div className="flex items-center gap-2 rounded-xl bg-zinc-50 p-3 text-sm text-zinc-400">
        ...
        <form onSubmit={handleAskCopilot} className="mt-5 flex gap-3">
          <input
            value={question}
          ...
          <button
            ...
            className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-green-700 text-white transition hover:bg-green-800"
