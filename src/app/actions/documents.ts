...
async function extractTextFromUpload(
  file: File,
): Promise<{ text: string | null; status: DocumentStatus }> {
  const ext = extensionOf(file.name);
  const isText =
  ...
  if (isDocx) {
    try {
      ...
      const text = normalizeExtractedText(docxResult?.value || "");
      if (!text) return { text: null, status: "ready" };
      return { text, status: "ready" };
    } catch {
...
function mapRow(row: {
  ...
  const excerpt = row.extracted_text
    ? row.extracted_text.slice(0, 180).replace(/\s+/g, " ").trim() +
      (row.extracted_text.length > 180 ? "…" : "")
    : null;
  return {
    id: row.id,
...
> {
  ...
  if ("error" in owned)
    return { success: false, error: owned.error ?? "Unknown error" };

  const { supabase, startup } = owned;
  const { data, error } = await supabase
    .from("knowledge_documents")
    .select(
      "id, file_name, mime_type, size_bytes, status, extracted_text, created_at, project_id",
    )
    .eq("startup_id", startup.id)
    .order("created_at", { ascending: false });
...
export async function uploadKnowledgeDocument(
  ...
  { success: true; document: VaultDocument } | { success: false; error: string }
> {
  ...
  if (file.size > MAX_BYTES)
    return { success: false, error: "File must be 25MB or smaller." };
  if (!isAllowedFile(file)) {
    return {
      success: false,
      error: "Allowed types: PDF, TXT, Markdown, DOCX.",
    };
  }
  ...
  if (insertError || !row) {
    ...
    return {
      success: false,
      error: insertError?.message || "Could not save document record.",
    };
  }
